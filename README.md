...
## Быстрый старт (docker + автозаполнение)
1. `make up` (или `docker compose up -d`)
2. Проверить, что исходные данные загружены: `make count` (10000 строк)
3. Проверить, что витрина создана и заполнена: `make dwcount` и `make check`

Если `dw.fact_sales` не существует (например, контейнер уже запускался раньше и volume не пересоздавали) — выполните `make dwinit` или `make reset`.

Используемые скрипты:
- `01_create_mock_data.sql` / `02_load_mock_data.sql` — создают и наполняют `public.mock_data`
- `03_dw_ddl.sql` / `04_dw_dml.sql` — создают измерения/факт в `dw.*` и заполняют их из `public.mock_data`
- `05_dw_checks.sql` — проверки/агрегации для пункта 10
- `06_source_analysis.sql` — примеры запросов для пункта 6
